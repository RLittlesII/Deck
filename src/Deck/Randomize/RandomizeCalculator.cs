...
/// </summary>
internal sealed class RandomizeCalculator<TObject, TKey> where TKey : notnull
{
    private RandomComparer<TObject, TKey> _comparer;
    ...
    /// </summary>
    public RandomizeCalculator(RandomComparer<TObject, TKey> comparer, SortOptimisations optimisations)
    {
        _comparer = comparer;
    ...
    /// <returns></returns>
    public IChangeSet<TObject, TKey> Load(ChangeAwareCache<TObject, TKey> cache) => Load(cache, false);
    ...
    /// <returns></returns>
    public IChangeSet<TObject, TKey> Load(ChangeAwareCache<TObject, TKey> cache, bool randomize)
    {
        ...
        _list = randomize ? cache.KeyValues.OrderBy(kv => kv, _comparer).ToList() : cache.KeyValues.ToList();

        var initialItems = _list.Select((t, index) =>
            new Change<TObject, TKey>(ChangeReason.Add, t.Key, t.Value, index));
    ...
    public IChangeSet<TObject, TKey> ChangeComparer(RandomComparer<TObject, TKey> comparer)
    {
        _comparer = comparer;
        ...
        if (_optimisations.HasFlag(SortOptimisations.ComparesImmutableValuesOnly))
        {
            ...
            if (index < 0)
            {
                throw new SortException(
                    "Current position cannot be found.  Ensure the comparer includes a unique value, or do not specify ComparesImmutableValuesOnly");
            }
        }
        else
        {
    ...
    private int GetInsertPositionLinear(IList<KeyValuePair<TKey, TObject>> list,
        KeyValuePair<TKey, TObject> item)
    {
